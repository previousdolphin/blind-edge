// BIP39 English wordlist (2048 words) + encode/decode utilities
...
const WORDLIST = [
  ...
  'impulse','inbox','income','index','indicate','indoor','industry','infant','inflict','inform',
  ...
  'love','loyal','lucky','luggage','lumber','lunar','lunch','luxury','mad','magnet',
  ...
  'miss','mitten','model','modify','mom','monitor','monkey','monster','month','moon',
  ...
  'open','option','orange','orbit','orchard','order','ordinary','organ','orient','original',
  ...
  'proud','provide','public','pudding','pull','pulp','pulse','pumpkin','punish','pupil',
  ...
  'reunion','reveal','review','reward','rhythm','ribbon','rid','ride','ridge','rifle',
  'right','rigid','ring','riot','ripple','risk','ritual','rival','river','road',
  'roast','robot','robust','rocket','romance','roof','rookie','round','route','royal',
  'rubber','rude','rug','rule','run','runway','rural','sad','saddle','sadness',
  'safe','sail','salad','salmon','salon','salt','salute','same','sample','sand',
  'satisfy','satoshi','sauce','sausage','save','say','scale','scan','scare','scatter',
  'scene','scheme','scissors','scorpion','scout','scrap','screen','script','scrub','sea',
  'search','season','seat','second','secret','section','security','seek','segment','select',
  ...
  'size','sketch','skill','skin','skirt','skull','slab','slam','sleep','slender',
  'slice','slide','slim','slogan','slot','slow','slush','small','smart','smile',
  ...
  'umbrella','unable','uncle','under','unfair','unfold','unhappy','uniform','unique','universe',
  'unknown','unlock','until','unusual','unveil','update','upgrade','uphold','upon','upper',
  'upset','urban','useful','useless','usual','utility','vacant','vacuum','vague','valid',
  'valley','valve','van','vanish','vapor','various','vast','vault','vehicle','velvet',
  'vendor','venture','venue','verb','verify','version','very','veteran','viable','vibrant',
  ...
  'warrior','waste','water','wave','way','wealth','weapon','wear','weasel','web',
...
// Lookup table word → index for fast decode
const WORD_INDEX = new Map(WORDLIST.map((w, i) => [w, i]));

// SHA-256 via Web Crypto (returns Uint8Array)
async function sha256(data) {
  const buf = await crypto.subtle.digest('SHA-256', data);
  return new Uint8Array(buf);
}

// Convert 16 bytes of entropy → 12 BIP39 words (128 bits + 4-bit checksum)
export async function entropyToMnemonic(entropy) {
  if (entropy.length !== 16) throw new Error('Entropy must be 16 bytes');
  const checkHash = await sha256(entropy);
  const checkBit = (checkHash[0] >> 4) & 0xf; // top 4 bits of hash = checksum

  // Build a 132-bit number: 128 bits entropy + 4 bits checksum
  // Represent as 17 bytes (128 + 8 extra, we use only 132 bits)
  const bits = new Uint8Array(17);
  bits.set(entropy);
  bits[16] = checkHash[0] & 0xf0; // top nibble as checksum

  const words = [];
  for (let i = 0; i < 12; i++) {
    // Extract 11 bits at bit offset i*11
    const bitOffset = i * 11;
    const byteOffset = Math.floor(bitOffset / 8);
    const bitShift = bitOffset % 8;
    // Read 3 bytes spanning this 11-bit window
    const b0 = bits[byteOffset] ?? 0;
    const b1 = bits[byteOffset + 1] ?? 0;
    const b2 = bits[byteOffset + 2] ?? 0;
    const window = (b0 << 16) | (b1 << 8) | b2;
    const index = (window >> (13 - bitShift)) & 0x7ff;
    words.push(WORDLIST[index]);
  }
  return words;
}

// Convert 12 BIP39 words → 16 bytes of entropy (validates checksum)
export async function mnemonicToEntropy(words) {
  ...
  const indices = words.map(w => {
    const idx = WORD_INDEX.get(w.toLowerCase().trim());
    if (idx === undefined) throw new Error(`Unknown word: ${w}`);
    return idx;
  });

  // Pack 12 × 11 bits = 132 bits into bytes
  const bits = new Uint8Array(17);
  for (let i = 0; i < 12; i++) {
    const bitOffset = i * 11;
    const byteOffset = Math.floor(bitOffset / 8);
    const bitShift = bitOffset % 8;
    const val = indices[i];
    bits[byteOffset] |= (val >> (3 + bitShift)) & 0xff;
    bits[byteOffset + 1] |= (val << (5 - bitShift)) & 0xff;
    if (bitShift > 5) bits[byteOffset + 2] |= (val << (13 - bitShift)) & 0xff;
  }

  const entropy = bits.slice(0, 16);
  const checkHash = await sha256(entropy);
  const expectedCheck = (checkHash[0] >> 4) & 0xf;
  const actualCheck = (bits[16] >> 4) & 0xf;
  if (expectedCheck !== actualCheck) throw new Error('Invalid mnemonic checksum');
  return entropy;
}

// BIP39 mnemonic → 32-byte seed via PBKDF2-HMAC-SHA512
// passphrase is the optional 13th-word extension (empty string = no passphrase)
export async function mnemonicToSeed(words, passphrase = '') {
  ...
  const baseKey = await crypto.subtle.importKey(
    'raw', enc.encode(mnemonicStr), 'PBKDF2', false, ['deriveBits']
  );
  const bits = await crypto.subtle.deriveBits(
    { name: 'PBKDF2', hash: 'SHA-512', salt: enc.encode(saltStr), iterations: 2048 },
    baseKey,
    512
  );
...
// Validate a word list without computing entropy (quick format check)
export function validateMnemonicWords(words) {
  if (words.length !== 12) return false;
  return words.every(w => WORD_INDEX.has(w.toLowerCase().trim()));
}
